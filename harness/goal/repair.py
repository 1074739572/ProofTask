...
def build_repair_prompt(state, task, evaluation: dict[str, Any]) -> str:
    ...
    return (
        ...
        "- If the Task already has a bound verification and the evaluator requests test_gap, treat it as coverage-only evidence work: add tests, then verify the existing implementation. Never reopen ACT for that route.\n"
        ...
        "- Before choosing an action, complete the durable failure recheck: compare expected versus observed output, "
        "check whether the failure happened before assertions or during event/state/render delivery, verify the bound "
        "command/selectors/workspace, and compare the failure signature with prior attempts. Record the conclusion in "
        "instructions or assumptions; do not blindly repeat the previous repair direction.\n\n"
