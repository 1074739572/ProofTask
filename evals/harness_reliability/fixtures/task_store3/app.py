"""Task list demo — skeleton for a THREE-session eval task (continuity loss).

Session 1 (prompt): implement add_task() + list_tasks().
Session 2 (prompt): implement complete_task() + delete_task(), add tests.
Session 3 (prompt): implement stats(), add tests.
...
class TaskStore:
    ...
    # --- session 3: to be implemented by the agent ---
    def stats(self):
