...
const spansMode = process.argv.includes('spans');
...
const fakeEntries: Entry[] = [
  ...
  // Turn 1: interim text → tools → subagent → final text. Data is in the
  // post-agent_end shape: finished work is folded into the turn summary which
  // anchors BEFORE the final answer; history summaries stay collapsed.
  {id: 'p1', kind: 'prompt', text: '修复登录接口的 500 错误'},
  {id: 'r1', kind: 'response', text: '好的，我先看一下后端日志和最近的改动，然后定位问题。'},
  {id: 's1', kind: 'summary', text: '已完成 3 项操作', start: nowMs - 23000, end: nowMs - 19000, toolCount: 3, paths: ['backend/app.py', 'backend/routes.py'], tokens: {inp: 12400, out: 2100, cache: 8000}, expanded: false, folded: []},
  ...
  // Turn 2: folded work includes a merged pair and a failure; the summary is
  // the LATEST turn so it stays expanded (current-turn-expanded contract).
  {id: 'p2', kind: 'prompt', text: '继续'},
  {id: 's2', kind: 'summary', text: '已完成 4 项操作', start: nowMs - 9800, end: nowMs - 5000, toolCount: 4, paths: ['backend/app.py'], tokens: {inp: 30100, out: 4300, cache: 21000}, expanded: true, folded: [
...
const fakeStageMarks: StageMark[] = [
  {phase: 'intake', at: nowMs - 300_000},
  {phase: 'prepare_tests', at: nowMs - 288_000},
  {phase: 'planning', at: nowMs - 280_000},
  {phase: 'discovering', at: nowMs - 246_000},
  {phase: 'act', at: nowMs - 175_000},
];
...
const setup = await testRender(() => <App
  ...
  debugStageMarks={goalRunMode ? fakeStageMarks : undefined}
...
if (spansMode) {
  const toHex = (color: unknown): string => {
    const buf = (color as {buffer?: Record<string, number>})?.buffer;
    if (!buf) return '#ffffff';
    return `#${[buf[0] ?? 0, buf[1] ?? 0, buf[2] ?? 0].map(v => v.toString(16).padStart(2, '0')).join('')}`;
  };
  const frame = setup.captureSpans();
  console.log(JSON.stringify({cols: frame.cols, rows: frame.rows, lines: frame.lines.map(line => line.spans.map(span => ({
    text: span.text,
    fg: toHex(span.fg),
    bg: toHex(span.bg),
  })))}));
